...
# =============================================================================
# GENERACIÓN AUTOMÁTICA DE VIEWS — GRINOLD-KAHN (α = vol · IC · score)
# =============================================================================
...
def generate_gk_views(returns: pd.DataFrame, assets: Sequence[str],
                      cov: pd.DataFrame, config: GKConfig,
                      forced_tickers: Optional[Sequence[str]] = None,
                      periods_per_year: int = 52,
    ...
    vol = pd.Series(np.sqrt(np.clip(np.diag(cov.to_numpy()), 0, None)),
                    index=cov.index).reindex(assets).fillna(0.0)

    views = []
    for a in assets:
        if a in forced:
            continue                              # FICO: retorno ya forzado
...
def run_profile(returns: pd.DataFrame, equity_assets: Sequence[str],
                ...
                views_as_alpha: bool = False) -> ProfileResult:
    """
    ...
    views_as_alpha: si True, las views ABSOLUTAS se interpretan como alphas
               residuales (Grinold-Kahn) y se re-anclan al equilibrio:
               Q_i ← Π_i + α_i. Las views relativas no se tocan.
               Usado por el modo automático.
    """
    ...
    # Re-anclar views-alpha (Grinold-Kahn): Q_i ← Π_i + α_i para views absolutas.
    # El generador GK produce q=alpha residual; aquí lo sumamos al equilibrio
    # del activo para obtener el retorno total que espera Black-Litterman.
    if views_as_alpha and views:
        anchored = []
        for v in views:
            if v.kind == "absolute" and v.asset in pi.index:
                anchored.append(View(
                    kind="absolute", asset=v.asset,
                    q=float(pi.loc[v.asset] + v.q),   # Π + α
